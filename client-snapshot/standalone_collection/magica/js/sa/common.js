define(function () {
	...
	var mod = {
		...
		cgti: function () { // commonGetTouchId. 端末によって効き具合が異なる.
	...
	var ua = {
		ios: mod.uaMatch('ipad', 'iphone', 'ipod'),
		ios6: mod.uaMatch('iphone os 6_'),
		ios7: mod.uaMatch('iphone os 7_'),
		ipad: mod.uaMatch('ipad'),
		ipod: mod.uaMatch('ipod'),

		android: mod.uaMatch('android'),
		isAndroidOs5: mod.uaMatch('android 5'),
		isAndroidOs4_4: mod.uaMatch('android 4.4'),
		isAndroidOs4_2: mod.uaMatch('android 4.2'),
		isAndroidOs4_1: mod.uaMatch('android 4.1'),
		isGalaxyNote: mod.uaMatch('sc-02e', 'sc-01g', 'sc-05d', 'scl22', 'scl24', 'sc-01f'),
		isGalaxysTab: mod.uaMatch('sc-01e'),
		isGalaxys2: mod.uaMatch('isw11sc', 'sc-02c', 'sc-03d'),
		isGalaxys3: mod.uaMatch('sc-03e', 'sc-06d', 'scl21'),
		isGalaxyNote2: mod.uaMatch('sc-02e'),
		isGalaxys3a: mod.uaMatch('sc-03e'),
		isGalaxyJ: mod.uaMatch('sc-02f'),
		isXperia: mod.uaMatch('is11s', 'is12s', 'so-01b', 'so-01c', 'so-01e', 'so-02e', 'so-03d', 'sol21', 'sol22'),
		isXperiaAX: mod.uaMatch('so-01e'),
		isArrows: mod.uaMatch('f-05d', 'f-10d', 'fjl'),
		isEluga: mod.uaMatch('p-02e'),
		isINFOBAR_A02: mod.uaMatch('htx21'),

		isNexus6: mod.uaMatch('nexus 6'), // Nexus 6.
		isNexus: mod.uaMatch('nexus 6', 'nexus 5'), // Nexus
		isNexus5x: mod.uaMatch('nexus 5x'), // Nexus
		isSO_04E: mod.uaMatch('so-04e'),
		isLowAnimeRate: mod.uaMatch('201f', '201m', '202f', '203sh', '206sh', '301f', '302hw', '302sh', '303sh', '403sc', '404kc', 'asus_t00p', 'dm015k', 'f-01f', 'f-02e', 'f-02f', 'f-04e', 'f-05e', 'f-06e', 'fjl22', 'htl21', 'htl22', 'htl23', 'l-01e', 'l-04e', 'l-05d', 'l-05e', 'lgl24', 'lgv31', 'n-02e', 'n-03e', 'n-04e', 'n-06e', 'nexus 10', 'nexus 7', 'p-02e', 'p-03e', 'sc-01f', 'sc-01g', 'sc-02e', 'sc-02f', 'sc-03e', 'sc-04e', 'sc-06d', 'scl22', 'scl24', 'sh-02e', 'sh-02f', 'sh-04f', 'sh-05f', 'sh-06e', 'sh-08e', 'shl21', 'shl22', 'shl23', 'so-01e', 'so-04d', 'sol21', 'wx10k'), //8レート
		iosVersion: function () {
			if (ua.ios) {
				return parseInt(navigator.userAgent.toLowerCase().split('os ')[1].substr(0, 1));
			}
		}
	};
	mod.ua = ua;
...
*/
	...
	function ajaxTimeoutError(reloadURL) {
		...
		if (commonJsErrorPopup) {
			...
			document.getElementById("commonJsErrorCautionText").innerHTML = "An connection error has occured.<span style='display:block;margin-top:20px;font-size:20px'>※Certain internal functions may still be processing.</span>"
			document.getElementById("commonJsErrorLinkBtn").innerText = "Reload";
			//ポップアップ立ち上げ
	...
	function menuGvgTimeSet() {
		...
		if (popupGuildTimer.innerText == "-1") {
			popupGuildTimerView.innerText = "Pause";
		} else {
