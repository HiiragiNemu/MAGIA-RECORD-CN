define([
	...
	var setNativeImg = function () {
		_.each(common.imgData, function (path, key) {
			...
			var elmBg = $('[data-nativebgkey=' + key + ']');
			if (elmBg) {
	...
	//
	// 	setTimeout(function () { $(e.currentTarget).removeClass("touch") }, 0);
	//
	...
	//
	// 	// エフェクトDOM生成する
	// 	var effectDom = $('<div class="commonEffect" style="display:none;">');
	// 	effectDom.css({
	// 		top: (prm.y - 128) + 'px',
	// 		left: (prm.x - 128) + 'px',
	// 	});
	// 	effectDom.append($('<div class="effect01">'));
	// 	effectDom.append($('<div class="effect02">'));
	// 	effectDom.append($('<div class="effect03">'));
	//
	// 	$("body").append(effectDom);
	// 	effectDom.show();
	// 	setTimeout(function () {
	// 		effectDom.remove();
	// 	}, 700);
